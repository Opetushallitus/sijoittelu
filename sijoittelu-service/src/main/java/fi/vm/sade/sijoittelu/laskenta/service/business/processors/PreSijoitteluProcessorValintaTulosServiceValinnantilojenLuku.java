...
public class PreSijoitteluProcessorValintaTulosServiceValinnantilojenLuku implements PreSijoitteluProcessor {
    ...
    public void process(SijoitteluajoWrapper sijoitteluajoWrapper) {
        ...
        sijoitteluajoWrapper.getHakukohteet().stream().flatMap(HakukohdeWrapper::hakukohteenHakijat).flatMap(h -> h.getValintatulos().stream()).map(v -> {
            ValintatuloksenTila valintatuloksenTila = valintatulokset.get(new MultiKey<>(v.getValintatapajonoOid(), v.getHakemusOid(), v.getHakukohdeOid())).stream().
                map(Valintatulos::getTila).findAny().orElse(ValintatuloksenTila.KESKEN);
