...
//
  ...
  public RestCasClient getHakukohderyhmapalveluCasClient(
      @Value("${sijoittelu-service.kouta-internal.username}") // TODO: arrange for proper credentials!!!
          String username,
      @Value("${sijoittelu-service.kouta-internal.password}")
          String password) {
    String ticketsUrl = UrlConfiguration.getInstance().url("cas.tickets");
    String service = UrlConfiguration.getInstance().url("hakukohderyhmapalvelu.auth.login");
    return new RestCasClient(
        CasConfig.CasConfig(
                username, password, ticketsUrl, service, CSRF_VALUE, CALLER_ID, "ring-session", "/auth/cas"));
  }
//
