...
public class HakukohdeDaoImpl implements HakukohdeDao {
    ...
    public List<Hakukohde> haeHakukohteetJoihinHakemusOsallistuu(Long sijoitteluajoId, String hakemusOid) {
        List<Hakukohde> hakukohteet = morphiaDS.find(Hakukohde.class)
                .field("sijoitteluajoId").equal(sijoitteluajoId)
                .field("valintatapajonot.hakemukset.hakemusOid").equal(hakemusOid)
                .enableSnapshotMode()
                .asList();
        hakukohteet.forEach(hakukohde -> {
            hakukohde.getValintatapajonot().forEach(valintatapajono -> {
                valintatapajono.setHakemukset(
                        valintatapajono.getHakemukset().stream()
                                .filter(hakemus -> hakemusOid.equals(hakemus.getHakemusOid()))
