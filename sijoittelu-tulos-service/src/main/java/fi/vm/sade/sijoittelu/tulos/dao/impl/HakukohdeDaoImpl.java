...
public class HakukohdeDaoImpl implements HakukohdeDao {
    ...
    public List<Hakukohde> haeHakukohteetJoihinHakemusOsallistuu(Long sijoitteluajoId, String hakemusOid) {
        ...
        String map = MongoMapReduceUtil.shallowCloneJs + " copy.valintatapajonot = copy.valintatapajonot.map(function(jono) { " +
                "jono = shallowClone(jono); " +
                "jono.hakemustenMaara = (jono.hakemukset || []).length; " +
                "jono.hakemukset = (jono.hakemukset || []).filter(function (hakemus) { return hakemus.hakemusOid == '"+hakemusOid+"' }); return jono }); emit(this.oid, copy) }\n";
